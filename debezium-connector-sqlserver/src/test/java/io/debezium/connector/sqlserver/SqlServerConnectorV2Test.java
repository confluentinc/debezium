...
public class SqlServerConnectorV2Test {
    SqlServerConnector_V2 connector;

    @Before
    public void before() {
        connector = new SqlServerConnector_V2();
    }
    ...
    public void testValidateUnableToConnectNoThrow() {
        ...
        config.put(SqlServerConnectorConfig_V2.HOSTNAME.name(), "narnia");
        config.put(SqlServerConnectorConfig_V2.PORT.name(), "4321");
        config.put(SqlServerConnectorConfig_V2.DATABASE_NAMES.name(), "sqlserver");
        config.put(SqlServerConnectorConfig_V2.USER.name(), "pikachu");
        config.put(SqlServerConnectorConfig_V2.PASSWORD.name(), "raichu");
    ...
    private Optional<ConfigValue> getHostName(Config config) {
        return config.configValues()
                .stream()
                .filter(value -> value.name().equals(SqlServerConnectorConfig_V2.HOSTNAME.name()))
                .findFirst();
    }

    @Test
    public void shouldReturnConfigurationDefinition() {
        assertConfigDefIsValid(connector, SqlServerConnectorConfig_V2.ALL_FIELDS);
    }
    ...
    protected static void assertConfigDefIsValid(Connector connector, io.debezium.config.Field.Set fields) {
        ...
        fields.forEach(expected -> {
            ...
            if (expected.equals(SqlServerConnectorConfig_V2.SCHEMA_HISTORY) || expected.equals(CommonConnectorConfig.TOPIC_NAMING_STRATEGY)) {
