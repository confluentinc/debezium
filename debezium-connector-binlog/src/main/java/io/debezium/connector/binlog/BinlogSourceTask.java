...
public abstract class BinlogSourceTask<P extends Partition, O extends OffsetContext> extends BaseSourceTask<P, O> {
    ...
    protected void validateBinlogConfiguration(Snapshotter snapshotter, BinlogConnectorConnection connection) {
        ...
        LOGGER.warn("Please ensure that the binlog is retained for at least 7 days. For disaster "
                    + "recovery scenarios, conenctor might be down for some time then resume from "
                    + "last read binlog position. In such cases, if the required binlog files are "
                    + "available, the conenctor can recover seamlessly. If not, data recovery "
