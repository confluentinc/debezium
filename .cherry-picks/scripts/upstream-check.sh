...
SKIP_FILE="${REPO}/.cherry-picks/upstream-skips.tsv"

echo "=== Upstream Skip Detection ==="
echo "Upstream tag: $UPSTREAM_TAG | Range: $START-$END"
echo ""

# Write header to skip file (TSV format for machine parsing)
echo -e "#\tSOURCE_SHA\tSUBJECT\tUPSTREAM_SHA\tMATCH_METHOD\tUPSTREAM_SUBJECT" > "$SKIP_FILE"
...
for i in $(seq "$START" "$END"); do
  ...
  if [ -n "$dbz_ticket" ]; then
    # Search upstream for commits with same DBZ ticket
  ...
  if [ -z "$upstream_sha" ]; then
    # Strip CC-XXXX prefix, PR number, and leading punctuation
  ...
  if [ "$status" = "SKIP" ]; then
    ...
    echo ""
    upstream_subject=$(git -C "$REPO" show "${upstream_sha}" --format="%s" --no-patch 2>/dev/null)
    echo -e "${i}\t${sha}\t${subject}\t${upstream_sha:0:10}\t${match_method}\t${upstream_subject}" >> "$SKIP_FILE"
...
echo ""
echo "Skip details written to: $SKIP_FILE"
