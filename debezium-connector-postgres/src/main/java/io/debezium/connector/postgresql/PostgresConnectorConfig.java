...
public class PostgresConnectorConfig extends RelationalDatabaseConnectorConfig {
    ...
    public static final Field CONNECTION_LIVENESS_PROBE_ENABLED = Field.create("connection.liveness.probe.enabled")
            ...
            .withImportance(Importance.MEDIUM)
            .withDescription(
                    "Run a dedicated background liveness probe (SELECT 1) on its own connection to detect a dropped/black-holed database connection within seconds and fail the task for a fast restart, instead of waiting for the OS TCP timeout. Enabled by default.")
