...
public class KafkaLogSnapshotCoordination implements SnapshotCoordination {
    ...
    private String encodeKey(Map<String, String> key) throws Exception {
        return keyMapper.writeValueAsString(new TreeMap<>(key)); // TreeMap = belt and suspenders
    }

    @Override
    public void start() {
        log.start(); // reads beginning→end, then starts background tailing thread
    }
    ...
    public void write(Map<String, String> key, Map<String, Object> data) throws Exception {
        ...
        log.sendWithReceipt(keyJson, valueJson).get(30, TimeUnit.SECONDS); // synchronous
        cache.put(key, new HashMap<>(data)); // eager local update
