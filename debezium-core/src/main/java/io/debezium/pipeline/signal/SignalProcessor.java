...
public class SignalProcessor<P extends Partition, O extends OffsetContext> {
    ...
    private void processSignal(SignalRecord signalRecord) {
        ...
        try {
            ...
            // Do not pass the parse exception 'e': it can echo the signal data. The signal id and the
            // exception class name are safe. (JacksonReader also has source-in-location disabled so its
            // location cannot carry the data.)
            LOGGER.warn("Signal '{}' has been received but the data cannot be parsed ({})", signalRecord.getId(), e.getClass().getSimpleName());
        }
        catch (InterruptedException e) {
