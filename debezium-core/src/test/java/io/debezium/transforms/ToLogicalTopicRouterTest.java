...
public class ToLogicalTopicRouterTest {
    ...
    public void shouldUseDefaultKeyFieldNameWhenNotExplicitlySet() {
        ...
        Schema keySchema = SchemaBuilder.struct()
                .name("server1.inventory.customers_shard_1.Key")
                .field("id", Schema.INT64_SCHEMA)
                .build();
        ...
        SourceRecord record = new SourceRecord(
                new HashMap<>(), new HashMap<>(), "server1.inventory.customers_shard_1", keySchema, key, null, null);
    ...
    public void shouldUseExplicitKeyFieldNameWhenSet() {
        ...
        props.put("topic.regex", "(.*)customers_shard(.*)");
        props.put("topic.replacement", "$1customers_all_shards");
        props.put("key.field.name", "custom_source_id");
        router.configure(props);
        ...
        SourceRecord transformed = router.apply(record);
        assertThat(transformed.keySchema().fields()).hasSize(2);
        assertThat(transformed.keySchema().fields().get(1).name()).isEqualTo("custom_source_id");
        assertThat(((Struct) transformed.key()).get("custom_source_id"))
                .isEqualTo("server1.inventory.customers_shard_1");
    }
